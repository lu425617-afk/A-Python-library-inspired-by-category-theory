...
identity = Mapping(lambda x: x, lambda xy: xy[1])
add = Mapping(lambda x: x[0] + x[1], lambda xyz: (xyz[1], xyz[1]))
...
assocL = Mapping(assocL_fwd, lambda xx: assocR_fwd(xx[1]))
assocR = Mapping(assocR_fwd, lambda xx: assocL_fwd(xx[1]))
...
linear = Mapping(linear_fwd, linear_rev)
...
sigmoid = Mapping(sigmoid_fwd, sigmoid_rev)
relu = Mapping(relu_fwd, relu_rev)
